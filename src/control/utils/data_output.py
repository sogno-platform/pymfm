...
def visualize_and_save_plots(mode_logic: dict, dataframe: pd.DataFrame, output_directory: str):
    ...
    if mode_logic["CL"] == CL.OPTIMIZATION_BASED:
        ...
        plt.figure(figsize=(12, 8))
        plt.plot(dataframe.index, dataframe['P_PV_controlled_kW'], label='P_PV_controlled_kW')
        plt.plot(dataframe.index, dataframe['P_PV_forecast_kW'], label='P_PV_forecast_kW')
        ...
        plt.plot(dataframe.index, dataframe['P_net_before_controlled_PV_kW'], label='P_net_before_controlled_PV_kW')
