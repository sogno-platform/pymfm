...
class Prepare:
    def read_forecast_input(self, data, forecast_level, interpolate_pv_first=True):
        ...
        pv_data["Timestamp"] = pd.to_datetime(
            pd.to_datetime(pv_data["Timestamp"]).dt.strftime("%Y-%m-%d %H:%M:%S")
        )
        ...
        for h_item in data["household_dyn"]["values"]:
            h_data = h_data.append(
                {
                    "Timestamp": h_item["Timestamp"],
                    "Static_Load_W": h_item["NumericValue"],
                },
                ignore_index=True,
            )
        ...
        time_start = pd.to_datetime(
            dateutil.parser.parse(data["start_forecast"]).strftime("%Y-%m-%d %H:%M:%S")
        )
        forecast_df = forecast_df.reset_index()
        time_end = pd.to_datetime(
            dateutil.parser.parse(data["end_forecast"]).strftime("%Y-%m-%d %H:%M:%S")
        )
