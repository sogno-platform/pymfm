...
class Forecast:
    ...
    def gute1_forecast(self,forecast_df, time_start,  time_end, pv_metadata, load_metadata, forecast_level,  Nscale = 1):
        ...
        while day_time_start < forecast_end:
            ...
            mask = (day_df.Timestamp >= day_time_start) & (day_df.Timestamp < day_time_end)
            day_df = day_df.loc[mask]
            day_forecast_data = self.forecast_calc(day_df, load_metadata['households'], load_metadata['avgconsumption'], Nscale)
            forecast_data = pd.concat([forecast_data, day_forecast_data], ignore_index=False)
    ...
    def forecast(self, forecast_df, time_start,  time_end, pv_metadata, load_metadata, forecast_level ):
        if forecast_level == 1:
            forecast_data = self.gute1_forecast(forecast_df, time_start, time_end, pv_metadata, load_metadata, forecast_level)
            return forecast_data
